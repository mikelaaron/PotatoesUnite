...
*TLS plan for 0.3.0 (not implemented), 2026-08-23.* When the Net leaves the LAN, the devices must speak HTTPS.

- **Client:** the Arduino core's `NetworkClientSecure` (WiFiClientSecure) with the core's Mozilla CA bundle via `client.setCACertBundle()` (`esp_crt_bundle`) — no pinned certificate, so the server can renew (Let's Encrypt) freely. Core 3.3.11 ships no standalone bundle file — the certificate data links in from the prebuilt IDF archives the moment `esp_crt_bundle_attach` is referenced (plus `tools/gen_crt_bundle.py` for custom subsets), so the cost shows up only in the linked app: budget roughly **+100 KB flash** for the full Mozilla bundle (a ≈170 KB “CMN_CRTS” subset exists if that ever hurts; both 3 MB / 2.5 MB OTA slots hold either with room) and **~45 KB heap per open TLS connection** — one more reason for the next point. Measure the real delta when 0.3.0 is built and record it here.
- **One client, shared.** Heartbeat/choice/register and the OTA manifest+image download go through a single shared client/session per task rather than one per request, so only one TLS handshake's RAM is ever in flight and OTA never doubles it.
- **Scheme decides.** `https://` URLs get the secure client; `http://` stays allowed and plain for LAN dev (the default `http://potatoes.local:8080` keeps working). The captive-portal server field accepts either.
- **Clock:** certificate validation needs time; the devices already NTP-sync before anything matters, but first-boot registration over HTTPS must wait for sync (add "waiting for the clock" to the status line if it shows).
