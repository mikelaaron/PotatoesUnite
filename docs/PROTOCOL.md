...
- **Which edition it prints.** From 07:30 local `bulletins.morning`; from 18:30 local `bulletins.evening` (falling back to the morning); at other times, or with neither present, `bulletin` (the latest printed). It reads `no`, `edition`, `headline` and the first two `items`.
- **The Question on paper.** With `choices` present the lower half is the Question (the Scene `line`) and the options; a short BOOT press moves the cursor, a long press (≥ 1.5 s) sends `/v0/choice {scene_rev, choice_id}`. A 409 Scene is rendered like a 200. The confirmed option is remembered by `id` across heartbeats, so the mark survives as long as the same `choices` come back; a different set of ids resets cursor and mark.
- **Refresh is rationed on the device**, not by the protocol: the panel refreshes only when the rendered page would change, at most once per 60 s, and not 23:00–06:00 local unless the headline changed. The server should not expect a Scene to be visible sooner than a minute after it changes, except after a key press.
