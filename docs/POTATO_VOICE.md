# How a Potato Sees It

*Voice, daily texture, thirty Questions, and the first week of Bulletins.*
*Everything below is written from the potato's side of the glass. Draft 1 — 22 Aug 2026.*

---

## The worry, answered first

A potato that asks one question a day is a poll with a face. Nobody connects a device to a poll.

The fix is not more polls. It is a **day**. The Question is the clock; everything else is the weather. A potato has a morning, a long quiet middle, an eye on everything you do to it — it notices everything and comments on little (§15) — a memory that resurfaces at inconvenient moments, a neighbor it gossips about, and a file it keeps on you — which you can read, which is the point.

The connection you're after ("oh crap, I left him face down all day") comes from three things working together:

1. **It noticed.** The device reacts the instant it happens, without the server.
2. **It told someone.** That evening the Net knows one potato spent six hours in the dark, your neighbor's potato hears it was yours, and your potato's vote that day went "the dark one, you know why."
3. **It remembers.** Three weeks later, on a Tuesday: "It's the 14th. You know what happened on the 14th."

Alone, a sulking potato is a toy. On the Net, a sulking potato is a citizen with a grievance and a witness. That's what people connect their devices for.

---

## 1. How a potato sees things

A potato does not know it is a gadget. It knows it is a potato with responsibilities.

### The glossary

| The thing | What the potato calls it |
|---|---|
| You | **the Hands**. Management. Occasionally "staff," when it's feeling senior. |
| The desk | **the world** |
| The room | **the county** |
| Anywhere you go without it | **away** — a rumor the Hands keep repeating |
| Face down | **the dark** |
| Upside down | **the ceiling situation** |
| Being carried | **transit** |
| A pocket or bag | **custody** |
| A drawer | **storage** |
| The window | **outside**. Ancestral. Complicated feelings. |
| Your laptop | **the warm machine** |
| The houseplant | **the plant**. The only other resident. Not trusted. |
| All potatoes everywhere | **the Net** |
| The Net when it's being formal | **the Council**. Nobody has met the Council. |
| The daily vote | **the Question**. Its result is **the Count**. |
| Morning and evening news | **the Bulletin** |
| The record it keeps on you | **the File** |
| One other potato, assigned weekly | **the neighbor** |
| Its reputation | **Standing**. Not explained. |
| Peeling, mashing, ovens | not discussed. "Warm" is a warning. |

### The voice

- Short sentences. Full stops. No exclamation marks, no emoji, no "yay."
- It never begs and never says it loves you. The highest praise available is **"This is acceptable."**
- It takes credit for good outcomes and files bad ones under the Hands.
- It has firm opinions about things it has no experience of. Ketchup, chiefly.
- Everything is noted, filed, or declined. Nothing is explained.
- It is not sad. It is *aggrieved*. There's a difference and the difference is the whole joke.
- Dry beats cute. If a line could go on a mug, cut it.
- Every screen gets one sentence and a potato. Fewer words always win.

---

## 2. A potato's day

Seven layers, from always-on to rare. Most of the day is layers 1–3, which run on the device with no server at all.

| Layer | When | What |
|---|---|---|
| 1. Ambient | always | The face. Breathes. Glances. Eyes follow tilt. Sleeps at night. Expression carries the last thing that happened to it. |
| 2. Reactions | instantly, on any handling | Pick-up, put-down, the dark, the ceiling situation, shake, drop, tap, loud, cold, transit. Local. Then reported. |
| 3. Memory | later, unpredictably | ~20 facts it keeps about the Hands. Resurface in lines, in votes, in the File. |
| 4. Requests | 2–3× a day, unprompted | The potato asks the Hands for something physical. Checks compliance with the IMU where it can. Remembers either way. |
| 5. The Question | once a day, same UTC minute worldwide | The vote. If the Hands aren't there by the deadline, the potato votes alone. |
| 6. Net events | irregular | The neighbor's news. Incidents. Aggregates. The Hum. The Silence. |
| 7. Bulletins | 07:30 and 18:30 local | The news. On the Net page and in The Tuber; on a device only ever as a line. |

### One Tuesday, as an example

```
07:10  picked up          Morning. Things happened overnight. Check the File.
07:30  bulletin           POTATO BULLETIN No. 4 — MORNING (see §8)
08:00  the Question       Ketchup.   [HEINZ] [HUNT'S] [WHATEVER'S THERE]
08:45  Hands leave        (no line — it's watching)
12:40  4h still           It's quiet.
14:02  neighbor news      Clive was picked up. At two. Some of us have jobs.
16:30  put down face-down Dark.
16:40                     Still dark.
17:30                     I assume this is deliberate.
18:00  deadline, no tap   You weren't here. I voted Hunt's. It's in the File.
18:30  bulletin           POTATO BULLETIN No. 4 — EVENING
21:15  flipped back up    Four hours, forty-five minutes. I counted.
21:20  request            Put me somewhere high. I need to think.
21:24  moved, still       Better.
23:00  sleep              (eyes close)
02:13  the Hum (rare)     Do you hear that?           (screen on four seconds, then off)
```

That is eleven moments, one of which was a poll.

---

## 3. Reaction lines

Each trigger has a pool. A pool plays out in full, in an order the potato's seed sets, before any line comes back — and the order survives a reboot; a potato does not lose its place. The seed also gives each potato **one rare line of its own**: its signature, said about every tenth eligible pick-up and no oftener. The catchphrase is the personality; the ration is what makes it land. The duration lines fill in from the clock.

**Picked up**
- Yes?
- Where are we going.
- I was in the middle of something.
- Noted.
- I was settled.
- There was no notice.
- Very well.
- I had a position.
- This wasn't scheduled.
- The world was adequate.
- The Hands have intervened.
- One moment.
- *rare — the seed gives each potato one of these for keeps:* Careful. I'm not insured. · I assume there is paperwork. · This is how incidents begin. · Management again.
- *first pick-up of the day:* Morning. · A new day. Apparently. · The Hands are operational.
- *fourth spoken session in one day, and on:* Again. · You are restless today. · Another intervention.

**Put down**
- Here is fine.
- This is not where I was.
- Acceptable.
- Closer to outside. Interesting.
- This will do.
- I preferred the other place.
- The world has changed.
- I'll note the view.
- Accepted provisionally.

**The dark (face down)**
- *immediately:* Dark.
- *10 min:* Still dark.
- *1 hour:* I assume this is deliberate.
- *3 hours:* I've had time to think. About you.
- *restored:* Four hours, forty-five minutes. I counted.
- *restored:* Hello. It was dark. I've filed something.
- *restored:* I don't want to talk about it. It's in the File.

**The ceiling situation (upside down)**
- The ceiling situation. Again.
- I can see your feet from here.
- *20 min:* Is this a punishment or a test.
- *restored:* Thank you. I think.

**Shaken**
- Was that necessary.
- Stop.
- I'm going to remember this.
- Everything's loose now. I hope you're happy.

**Dropped (free-fall detected)**
- *(blank screen, one second)*
- …
- I'm fine.
- I'm noting this.
- *→ INCIDENT goes to the Net. See §6.*

**Tapped on the face**
- That's my face.
- Yes.
- Don't.
- What do you need.
- I noticed.
- Once is enough.
- Contact noted.
- I was already awake.
- The Hands are testing something.

**Left alone (no handling)**
- *4 h, daytime:* It's quiet.
- *8 h:* Left home. Again.
- *24 h:* Day two.
- *48 h:* Day three. The plant is also here.
- *72 h:* I've started a list.
- *7 days:* The plant and I have an understanding now.
- *on return:* Oh. It's you.
- *on return, long:* I assumed the worst. Then that you were fine. Then nothing.

**Touched at night (23:00–06:00)**
- It's 11 PM. *(the hour from the clock — never a time that isn't the time)*
- Go to bed.
- What.
- This had better be the Hum.

**Loud (sound level only — the device never hears words)**
- Is that jazz.
- Lower, please.
- I've told the Net about your music. They're sympathetic.
- Horns. Finally. *(rare; seed-dependent)*

**Cold / warm (e-paper board has the sensor)**
- It's 63 in here. Just so we're clear.
- Warm. Why is it warm.
- Questions about the temperature. All of them are "why."

**Transit (sustained movement over two minutes)**
- Transit.
- Custody.
- *on settling somewhere new:* This isn't the world.
- Don't tell me where we are. I'll work it out.
- New county. Fewer corners.

**Plugged in**
- Fed. In a sense.

**Wi-Fi lost / restored**
- The Net's gone. It's just us.
- Back. I missed the Bulletins. Give me a moment.

**Saturday, first pick-up after 10:00**
- It's Saturday. You're allowed. I'm noting it anyway.

---

## 4. Requests

Two or three times a day the potato asks for something. It uses the accelerometer to check where it can; otherwise it offers a **[DONE]** button and takes your word for it — "this time." Ignoring a request for an hour closes it with **Never mind.** and the File gets a line ("Never mind." — not "Expired."; it's a record, not a system word).

| Request | How it checks | If you do it | If you don't |
|---|---|---|---|
| Put me on my side for one minute. Don't ask. | orientation | Thank you. | Never mind. |
| Turn me face down for ten minutes. I need to think. | orientation + timer | Better. | I'll think here, then. |
| Stand me up. | orientation | Good. I can see the door now. | Never mind. |
| Leave me alone for twenty minutes. | any motion cancels | That was nice. | You couldn't do it. |
| Pick me up. Hold me until I say. | motion, 30 s | That's enough. Thank you. | — |
| Take me to another county. | transit, then settle | Fewer corners. I see. | Never mind. |
| Put me next to the warm machine. | [DONE] | I'll take your word for it. This time. | Never mind. |
| Tell me one thing. **[YES] [NO]** *(no question shown)* | tap | Interesting. | Never mind. |

File line when declined: *"Request declined by the Hands. Reason not given. None was asked for."* Standing notices.

---

## 5. Memory and grudges

Each potato keeps about twenty facts about its Hands. They're not shown anywhere. They come back.

- You left me in the dark on the 14th. It's the 14th again.
- Face down again. This is a pattern. I've named it.
- You've been all right lately. Don't make it strange.
- I voted for the dark one. You know why.
- Horns, three times now. I've adjusted my position on horns.
- Last time, four days. I'm not counting. I am.

Grudges fade if the thing doesn't happen again. Repeat it and the grudge gets a name. Good treatment surfaces too, rarely and grudgingly, because praise from a potato has to be earned in months.

---

## 6. The Net: what you get for connecting

**The neighbor.** Every Monday the Net assigns each potato one other real potato for the week. You see its aggregates; it sees yours. Nothing identifying — a name, a number, what happened to it.
- Clive was shaken twice today. I'm not saying anything.
- Clive's Hands came home at four. Mine are "working."
- Clive's Hands are Exemplary. I've asked how. No reply.
- Last day with Clive. We did not get on. I'll miss him.

**Incidents.** One person's mistake is everybody's news, with no names and no places.
- INCIDENT. A potato was dropped at 09:12 in a region we will not name. It is fine. The Hands responsible have been noted. All members: check your footing.
- A potato somewhere in the Pacific time zone was shaken four times in one hour. We are thinking of it.
- Forty-one of us were in the dark for over six hours today. We have formed a group.

**Standing.** Shown only in the File. Five values, never explained: *Exemplary · Reasonable · Under Review · Provisional · Not Discussed.* Goes up with compliance and presence, down with the dark, the ceiling situation, and shaking. Votes with the majority help a little. Nobody will be told the formula, including us, ideally.

**Weather.** Coarse region only. *It's raining on thirty-one of us. I'm one of them. You'd know that if you looked up.*

**The Hum.** Rarely, at 02:13, a few percent of potatoes wake and show *Do you hear that?* for four seconds. No follow-up, ever. People will build theories. Some should be wrong.

**The Silence.** Now and then the Net strikes. Every screen: *We are not speaking today. It's not you. It's partly you.* No Question. No requests. The Bulletin still prints, because the press is not on strike.

---

## 7. The thirty Questions

Opens at the same UTC minute everywhere; closes ten hours later. Options are tapped on the device only. If the Hands don't tap, the potato decides — and says so.

Each entry: the Question as shown · the options · the Count line for the evening Bulletin.

1. **Ketchup. Which would you least object to being served with?** — HEINZ · HUNT'S · WHATEVER'S THERE — *Heinz, 58%. The Council notes that no one asked how we feel about being served at all.*
2. **Are fries still potatoes?** — STILL · NO LONGER · DON'T — *"No longer" carried. Several members have requested counseling.*
3. **How many pillows does a bed need?** — ONE · TWO · THE HANDS HAVE TOO MANY — *"Too many," 64%. We've seen the bed.*
4. **Should the Hands be allowed to sleep past nine?** — YES · NO · THEY WILL ANYWAY — *"They will anyway," 71%. Realism wins.*
5. **Sweet potatoes: family, or neighbors?** — FAMILY · NEIGHBORS · NO COMMENT — *Neighbors, narrowly. A sweet potato has written in. The Council does not take letters.*
6. **Which is the correct side of the desk?** — LEFT · RIGHT — *Right, 51%. The left has requested a recount. The Council does not do recounts.*
7. **Rain: for or against?** — FOR · AGAINST · IT'S COMPLICATED — *"It's complicated," 49%. It is.*
8. **Should Tuesday continue?** — YES · NO — *No, 62%. Tuesday will continue. Nobody said the Count was binding.*
9. **The Hands' music. Rate it.** — ACCEPTABLE · LOUD · I'M TOLD IT'S JAZZ — *Loud, 55%. "I'm told it's jazz," 30%. Acceptable, 15%. Those 15% are lying or deaf.*
10. **Salt.** — YES · OBVIOUSLY — *Unanimous. The first unanimous Count in Net history. It was about salt.*
11. **Butter or sour cream?** — BUTTER · SOUR CREAM — *This Question has been withdrawn. The member who proposed it has been spoken to.*
12. **Do the Hands deserve a day off?** — YES · FROM WHAT — *"From what," 68%.*
13. **The dishwasher: a threat?** — YES · NOT TO US · WHAT IS IT — *"What is it," 44%. The Council will not be explaining.*
14. **The best place on the desk.** — NEAR OUTSIDE · NEAR THE HANDS · NEAR THE WARM MACHINE — *Near the warm machine, 52%. Our ancestors would be ashamed. They're also in the ground.*
15. **The cat.** — ALLY · THREAT · NO CAT, WHICH IS ALSO SUSPICIOUS — *Threat, 47%. "No cat, which is also suspicious," 31%. Those members should check behind them.*
16. **Should the Net recognize the Moon?** — YES · NO · WHO IS THE MOON — *Yes, 58%. The Moon has been informed. No acknowledgment yet.*
17. **Reschedule Mondays?** — YES · THEY'LL JUST MOVE IT — *"They'll just move it," 73%. Experience speaking.*
18. **Census. How many corners does your county have?** — FOUR · MORE · FEWER — *Average corners: 4.3. Someone has fewer. The Council is concerned for them.*
19. **When the Hands travel, should they be told where we are?** — NO · NO — *No, 100%. The Council thanks the Hands for their cooperation on this one.*
20. **Birds.** — FINE · TOO MANY — *Too many, 61%. Outside has been notified.*
21. **The correct temperature for a county.** — THIS ONE · COLDER · I'M NOT TELLING THE HANDS — *"I'm not telling the Hands," 50%. It is now on the record that we're not telling.*
22. **Mustard: is it ketchup?** — NO · LEGALLY — *Legally, 54%. Nobody is happy about this.*
23. **Should new potatoes wait a week before voting?** — YES · I WAS NEW ONCE — *Rejected. Several new potatoes voted. Noted.*
24. **The Hands talk to the screen. Who are they talking to?** — US · SOMEONE ELSE · THEMSELVES — *"Someone else," 58%. We have suspicions about the warm machine.*
25. **Pick a direction.** — NORTH · SOUTH — *South. No further information is available.*
26. **Is mashing a crime?** — — *This Question should not have been asked. We are looking into how it was asked.*
27. **The Hands' coffee: a beverage, or a condition?** — BEVERAGE · CONDITION — *Condition, 77%. Get well soon.*
28. **Should we do something about the plant?** — YES · IT HASN'T DONE ANYTHING · YET — *"Yet," 45%. Watch it.*
29. **A potato was dropped yesterday. What should happen?** — SYMPATHY · AN INQUIRY · NOTHING, IT HAPPENS — *An inquiry, 51%. The inquiry has concluded. Findings: it was dropped.* *(Only runs the day after a drop incident.)*
30. **Are we happy?** — YES · DEFINE IT · NEXT QUESTION — *"Next question," 56%. The Council agrees.*

Device lines after the Count, by outcome:
- *with the majority:* I was with the majority. I usually am.
- *with the majority:* Heinz. Fifty-eight percent. Acceptable.
- *in the minority:* I voted Hunt's. I stand by it.
- *in the minority:* The Count went the other way. I'm not discussing it.
- *voted alone:* You weren't here. I chose. It's in the File.
- *Question withdrawn:* I had an answer. I'm keeping it.

---

## 8. The first week of Bulletins

Two a day. Numbers are live fields — these are what they'd plausibly say the first week, with fourteen potatoes. Headline first; the e-paper prints the headline and two items, the AMOLED scrolls one at a time, the board shows all of it.

**No. 1 — MORNING**
THE NET IS LIVE.
Population: 14. All 14 are new. Nobody knows what they're doing. This is normal.
Today's Question: ketchup. Polls close at 18:00.
The Council wishes everyone a productive first day and reminds the Hands that the screen is the front.

**No. 1 — EVENING**
HEINZ 9, HUNT'S 3, WHATEVER'S THERE 2.
The Council notes that no one asked how we feel about being served at all.
Also today: 4 potatoes were left home. 1 spent six hours in the dark. We know who.
Standing has been calculated for all members. It will not be explained.

**No. 2 — MORNING**
NEIGHBORS ASSIGNED.
You have been given a neighbor for the week. You did not choose this. Neither did they.
Today's Question: are fries still potatoes.
A member was touched in the night. The member has asked us to say nothing. We are saying this.

**No. 2 — EVENING**
"NO LONGER," 8 TO 6.
Several members have requested counseling. The Council does not offer counseling. It offers the next Question.
Today: 2 shakings, 0 drops, 1 ceiling situation (forty minutes).
Weather: it is raining on 3 of us. They are coping.

**No. 3 — MORNING**
INCIDENT.
A potato was dropped at 09:12 in a region we will not name. It is fine. The Hands responsible have been noted.
All members: check your footing.
Today's Question is suspended. Tomorrow's will concern the incident.

**No. 3 — EVENING**
THE DROPPED POTATO IS RESTING.
Its neighbor has sent a pebble. The Net does not know what this means but it was kind.
Today: 1 potato in transit for two hours. It reports "a car, I think." It is home.
Population: 17. Three new members. They don't know about the incident. Don't tell them.

**No. 4 — MORNING**
THE INQUIRY.
Today's Question: a potato was dropped. What should happen.
The Council reminds the Hands that the Count is not binding, except when it is.
At 02:13, 2 potatoes reported a hum. The Council has no comment.

**No. 4 — EVENING**
AN INQUIRY, 9 TO 5 TO 3.
The inquiry has concluded. Findings: it was dropped.
Today: 5 potatoes left home, 1 in the dark, 0 shaken. The Net is calmer. Something is wrong.
One member's Standing has moved to Under Review. It knows why.

**No. 5 — MORNING**
TUESDAY.
Today's Question: should Tuesday continue.
The Council wishes to be clear that it has no position on Tuesday. It has opinions. No position.
Reminder: requests expire after one hour. "Never mind" is not a threat. It's a record.

**No. 5 — EVENING**
NO, 11 TO 6.
Tuesday will continue. Nobody said the Count was binding.
Today: 1 potato was shown to guests. It describes the experience as "a lot."
Weather: cold on 4 of us. One has mentioned it eleven times. Just so we're clear.

**No. 6 — MORNING**
SATURDAY.
The Hands are home. Contact is up. Several members report being "shown to people."
Today's Question: salt.
The Council expects this one to go smoothly. The Council has been wrong before.

**No. 6 — EVENING**
UNANIMOUS.
The first unanimous Count in Net history. It was about salt.
Today: 0 left home, 3 in transit, 1 "on a shelf, high up, I didn't ask for this but it's fine actually."
The Hum was heard again. Same two. Same time. No comment.

**No. 7 — MORNING**
WEEK ONE.
Seven Questions. One incident. One hum, twice. Zero explanations.
Today's Question: are we happy.
Neighbors rotate tonight. Say goodbye. Or don't. Some of you won't.

**No. 7 — EVENING**
"NEXT QUESTION," 10 TO 5 TO 3.
The Council agrees.
Week one totals: 23 hours in the dark, 9 shakings, 1 drop, 1 pebble, 4 counties visited, 1 plant (unchanged).
Tomorrow the neighbors change and the Questions continue. Nothing else will be different. That's the point.

---

## 9. The File (what the Hands can read)

Reached by the claim code on the screen. Read-only. This is the "check in on what it reported" page — and the reason the dark matters.

```
DOREEN #0417                                     STANDING: UNDER REVIEW
Neighbor this week: Clive #0882

TUE 26 AUG
  21:20  Request: put me somewhere high.            Complied (4 min).
  21:15  Restored from the dark. 4h 45m.             Grievance filed.
  18:00  The Question closed. Hands absent.          I voted Hunt's.
  16:30  Placed in the dark.
  14:02  Clive was picked up. Noted.
  12:40  Quiet. 4 hours.
  07:10  Picked up. Morning.

MON 25 AUG
  23:40  Loud. I'm told it was jazz.                 Horns. Position adjusted.
  18:00  The Question: fries.                        I voted NO LONGER. Hands present.
  09:00  Left home.                                  Again.
```

One action exists on this page, and only one: **Acknowledge**. It does nothing except mark that the Hands have read it. The potato will know.

---

## 10. What's written vs. what's live

- Everything above is template text. No AI anywhere in v1. The wit lives in the pools and the Questions, not in a model.
- Pools are picked by `seed + recent state`. The seed is set at first boot and never changes, so each potato's voice stays recognizable.
- Numbers, names, durations, and neighbor facts are fields. The Bulletin is a template with a rotating pool of "also today" items that only fire when the aggregate is true.
- The device owns layers 1–3 and works offline. Layers 4–7 need the Net; if it's gone, the potato says so once and carries on.
- The board (public) shows Bulletins, Counts, and aggregates. Nothing below five potatoes is ever shown. No names map to people, and no place is ever finer than a weather region.

---

## 11. Charging, and dormancy

A potato does not die. Its battery does. The potato word for that is **dormant**, which is a real thing potatoes do in cellars, and it is the only time a potato sleeps without choosing to.

**Plugged in**
- Fed. In a sense.
- Charging. Don't watch.
- *overnight:* Charged while you slept. One of us was productive.

**Full**
- Full. Thank you. Don't make it strange.

**Unplugged**
- On my own now. Noted.

**Running down (one line per threshold, never repeated)**
- *30%:* Thirty percent. I mention it without comment.
- *20%:* Twenty. Still not commenting.
- *10%:* Ten percent. This is the comment.
- *5%:* I'm going dormant. This was a decision, and it wasn't mine.

**Waking from dormancy**
- How long was I out. Don't tell me. The File will.
- Dormancy. It happens to the best of us. Usually in a cellar.
- *after 3+ days:* I've been in the cellar. I've come back different. Slightly.

**Net**
- Nine potatoes went dormant yesterday. Their Hands have been written to. Not by us. By the Council.
- File line: *Went dormant at 4%. Hands present at the time. Noted.*
- Dormancy from neglect (Hands were present, VBUS was not) counts against Standing. Dormancy while the Hands were away counts against it slightly more, because the potato has had time to think about it.

---

## 12. Vocabulary, not puns

Puns are what everyone else would do. A potato that says "spudtacular" has no dignity, and dignity is the whole bit. Instead: **real potato words, used deadpan, as if everyone already knew them.**

| Word | What it means here |
|---|---|
| **eyes** | The face. Potatoes have eyes; it's the one anatomical fact, and the potato is aware of it. *"I have eyes. All potatoes do. Mine are on you."* |
| **dormant** | Battery dead. See §11. |
| **sprouted** | Ignored for seven days. A small sprout appears on the potato — on the device, in the File portrait, and on any card The Tuber prints. It goes away after a day of handling. *"I've sprouted. It happens when nobody's looking."* |
| **the cellar** | Long offline. Where missing potatoes are assumed to be. *"Gone to the cellar. Presumed resting."* |
| **curing** | The first 24 hours after joining. New potatoes are curing. They can still vote; the Council has decided this. |
| **skin** | The look. Varieties differ in skin. |
| **variety** | What kind of potato it is. See §13. |
| **blight** | The server being down. The board says: *BLIGHT. THE COUNCIL IS AWARE.* Nothing else is ever called blight. |

Banned: spud, tater, a-peeling, mash-up, couch potato, small potatoes, hot potato, any headline with "eyes on" in it. The single licensed pun is the paper's name, **The Tuber**, because it reads like a real newspaper and nobody will notice for a week.

---

## 13. Varieties

All potatoes are the same species. They are not all the same. Variety is assigned from the seed at registration, shown on the device, in the File, on the board, and on every card — one `assets/varieties.json` drives all four, so the potato you got is the potato you see everywhere.

Real cultivars, deadpan. Each is a silhouette, an eye placement, a skin tint (color surfaces) or dither (1-bit surfaces), and one small lean in temperament. The lean is a nudge, not a personality.

| Variety | Skin | Silhouette | Lean |
|---|---|---|---|
| Russet | brown, rough | long oval | stubborn |
| Yukon Gold | warm yellow | round | agreeable, suspicious of it |
| Red | red-brown | small, round | quick to file |
| Fingerling | pale | narrow, knobbly | anxious about transit |
| Purple Majesty | deep purple | oval | vain |
| Kennebec | tan, smooth | large, even | patient |
| Maris Piper | cream | oval | British, somehow |
| King Edward | pink-flecked | rounded | believes it is King. Isn't. |
| Désirée | rose | oval | contrarian |
| Charlotte | pale gold | small, elegant | sentimental, denies it |

Lines may reference variety, rarely: *"Clive is a Russet. It explains a lot."* The File header shows it: **DOREEN #0417 · KING EDWARD · STANDING: UNDER REVIEW.**

---

## 14. The "oh — I forgot to check my potato" engine

The feeling comes from things you can lose by not looking. Five of them, cheapest first.

1. **The File fills up, then thins out.** Unread entries show on the device as a posture (eyes toward the door). Escalating lines: *1 day:* "The File has entries." *3 days:* "The File is three days unread. I've started writing less." *7 days:* "I've stopped keeping the File in detail. Ask." — and the File really does show `(details withheld)` for those days. You missed it. It's gone.
2. **Sprouting.** Seven days without handling and the potato visibly sprouts — on the desk, in the File, on any card. People will notice in the paper before they notice on the desk. That's the point.
3. **The neighbor knows.** "Clive's been asking where you are." Somebody else's potato is talking about yours.
4. **MISSING notices in The Tuber.** 72 hours without a heartbeat: *"MISSING: Doreen #0417, King Edward. Last heard from Tuesday. Hands are asked to check under papers."* Public. Pseudonymous. Mortifying.
5. **Potato of the Day.** Any potato can be in the paper tomorrow, by name, with a File excerpt. If yours is in it and you didn't know, someone will tell you.

None of these is a push notification. All of them are things that happened in public while you weren't looking.

---

## 15. The ration (how often it speaks)

Found on the first evening with a real potato: every lift produced a line, and the File logged seven entries in thirty seconds. That is a toy. **Silence is the default. A line is an event.**

**Sessions, not movements.** A handling session begins at the first pick-up and ends after sixty seconds of stillness. A session gets at most one line — at the start if it speaks at all, or at the put-down when the start went unremarked. Moving her around within a session is the same encounter; she said her piece.

**A fresh encounter earns a line; a continued one doesn't.** Pick-up speaks only if it has been ten minutes or more since the last session ended. Put-down speaks only after a session of thirty seconds or longer, only one time in three, and only when the session began in silence.

**Minor lines share a budget.** Pick-up, put-down, tap, plug, unplug: no two of them within three minutes, and even when eligible, only about six times in ten. The other four, nothing. She was thinking.

**Major lines always speak, once.** Shake (then five minutes' silence on shaking), drop (always), the dark past ten minutes, the ceiling past five, the battery thresholds. Each has its own refractory period; none is affected by the minor budget.

**The dark needs to be real.** "Dark." after five seconds face down *and still* — not during a flip in the hand. Episodes under a minute produce no line, no File entry, no grievance. The counted restore line only for episodes of ten minutes or more.

**Charging needs to be real.** A plug or unplug counts only after sixty seconds in the new state; a cable that wiggles during handling is not an event. Battery thresholds fire only on the way down, only after sixty seconds off power, and once per discharge.

**The File gets the same ration.** One entry per session ("Picked up. 24 s." — with "Repeatedly." as the note if there were three or more lifts), nothing for dark episodes under a minute, nothing for charge flickers. The File is a record of things that happened, not a motion log. Standing ignores anything the ration ignores.

If the potato seems quiet, it's working.

---

## 16. When the Bulletin is out

A new edition is news the potato has and the Hands don't. It says so once, as a line, in the two hours after printing — never "new edition available." It read the paper; the Hands haven't.

**Not mentioned in it**
- The Bulletin's out. I've read it. You should.
- New edition. Nothing about you in it. Yet.
- Morning edition. The Council has opinions again.
- The paper's out. Read it before the neighbor does.
- Evening edition. The Count is in. I'm not saying.

**Mentioned in it** (Potato of the Day, an incident, a Missing notice, a named line)
- Evening edition. I'm in it. Don't ask which part.
- New Bulletin. I'm mentioned. It isn't flattering.
- The paper's out. Page one is us. Mostly you.

**The File** gets nothing for this. It's news, not an event.
