...
- Look: the potato lies down, brown, asymmetric, two dark dimple-eyes, no mouth, no limbs, smooth and anti-aliased on black. Spec and mock: `.Codex/agents/art.md` + `assets/potato-look-v1.svg`. A coarse pixel grid on a featureless blob reads as "low-res," not "retro" (see the creature repo lessons); old-school lives in the type and the layout.
