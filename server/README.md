...
- `data/broadcasts.json` — an array of scheduled overrides:
  ...
  - `{"id":"midday","type":"daily_line","local_at":"12:30","local_days":["tue","thu","sat"],"duration_min":10,"lines":["…"]}` — one authored, non-interactive line in each listed local window. Omit `local_days` to run daily. Day ids are `sun` through `sat`; they are calculated after applying the potato's reported `utc_offset_min` (UTC when absent). The seed walks the eligible pool without a repeat until every line has played. The line briefly overlays the Scene but does not move the Question's 13:00/23:00 UTC clock, enter the File, or print in a Bulletin. `duration_min` must be 1–120 and every line must be ≤ 60 characters. Optional absolute `from`/`to` bounds can enable or retire the schedule.
  - Daily-line entries may be plain strings or `{"id":"N6","when":"neighbor_recent_6h","text":"{neighbor} checked in with the Net today."}`. Conditional entries are eligible only when their fact is current; an unconditional entry is the deterministic fallback. Supported conditions are `neighbor`, `neighbor_recent_6h`, `neighbor_completed_handling_today`, `neighbor_completed_transit_today`, `neighbor_quiet_4h`, `self_quiet_4h`, `self_still_since_0900`, and `self_handled_3_today`. A continuous-heartbeat fact follows the protocol's 120-second cadence and tolerates one missed beat (a gap over five minutes resets coverage). Completed handling is counted from rationed File episodes, never raw pickup edges. Neighbor names are server-filled; an absent neighbor can never leave a `{neighbor}` on screen.
